...
function writeTable(contributors: User[], perLine = 5): void {
  ...
  lines.push('')
  lines.push(
    `This list is auto-generated using \`rpx\`. It shows the top ${PAGE_LIMIT} contributors with > ${COMPLETELY_ARBITRARY_CONTRIBUTION_COUNT} contributions.`
  )
  lines.push('')
...
export const generateContributors = () => {
  main()
    .then(() => Logger.success(t('Generate.Contributors.Success')))
    .catch((error) => {
      Logger.error(t('Generate.Contributors.Failed'))
      Logger.info(error)
      process.exitCode = 1
    })
}
